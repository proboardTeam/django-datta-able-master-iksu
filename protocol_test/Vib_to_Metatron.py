...
def main(sensor, duration):
    ...
    time_list = list(chain(dates, datesT, datesb, datesAD_X, datesAD_Y, datesAD_Z))

    #10개 리스트 제작 - AD = anormaly detection; predict error
    Xrms_list, Yrms_list, Zrms_list, Xkurt_list, Ykurt_list, Zkurt_list, Temp_list, X_AD, Y_AD, Z_AD, battery_list = ([] for i in range(11))

    for i in range(len(dates)):
        time_list[i] = round(datetime.datetime.strptime(time_list[i], "%Y-%m-%d %H:%M:%S").timestamp()*1000+3600000*9)
        # X axis
        if axes[i]==0:
    ...
    it = [Xrms_list, Yrms_list, Zrms_list, Xkurt_list, Ykurt_list, Zkurt_list, Temp_list, X_AD, Y_AD, Z_AD, battery_list, time_list]
    the_len = len(time_list)
    if not all(len(l) == the_len for l in it):
        raise ValueError('All lists must have the same length')

    try:
        data2 = [{"serviceId":serviceId, "deviceId":deviceId, "timestamp": d, "contents":{"XRms": x,"YRms": y, "ZRms": z, "gKurtX": kx, "gKurtY": ky, "gKurtZ": kz, "XaiPredError":adX, "YaiPredError":adY, "ZaiPredError":adZ, "BoardTemperature": t,"BatteryState":b}}
             for (d, x, y, z, kx, ky, kz, adX, adY, adZ, t, b) in list(zip(time_list, Xrms_list, Yrms_list, Zrms_list, Xkurt_list, Ykurt_list, Zkurt_list, X_AD, Y_AD, Z_AD, Temp_list,battery_list))]
    except:
