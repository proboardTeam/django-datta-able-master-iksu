# Using Python-opcua to retrieve and plot vibration data of a specific time period. 

import time, sys, pytz, logging, datetime
...
class OpcUaClient(object):
    ...
    def read_raw_history(self,
                         uaNode,  starttime=None,  endtime=None,  numvalues=0,  cont=None):
...
class DataAcquisition(object):
        ...
        with OpcUaClient(serverUrl) as client:
            ...
            (values, dates) = \
                DataAcquisition.get_endnode_data(
                        client=client,  endNode=sensorNode,  starttime=starttime,  endtime=endtime )
        return (values, dates)
    ...
    def get_endnode_data(client, endNode, starttime, endtime):
        dvList = DataAcquisition.download_endnode(
                client=client,  endNode=endNode,  starttime=starttime,  endtime=endtime )
        dates, values = ([], [])
        for dv in dvList:
    ...
    def download_endnode(client, endNode, starttime, endtime):
        ...
        while True:
            ...
            partial, contId = client.read_raw_history(
                uaNode=endNode,  starttime=starttime,  endtime=endtime,  numvalues=numvalues,  cont=contId)
            if not len(partial):
...
if __name__ == "__main__":
    ...
    macId = '95:b0:30:c5'
    starttime = pytz.utc.localize( datetime.datetime.strptime("2022-03-20 00:00:00", '%Y-%m-%d %H:%M:%S') )
    endtime = pytz.utc.localize( datetime.datetime.strptime("2022-03-25 00:00:00", '%Y-%m-%d %H:%M:%S') )

    # acquire history data
    (values, dates) = DataAcquisition.get_sensor_data(
        serverUrl=serverUrl, macId=macId, browseName="accelerationPack", 
        starttime=starttime, endtime=endtime)
    (temperatures, datesT) = DataAcquisition.get_sensor_data(
        serverUrl=serverUrl, macId=macId, browseName="boardTemperature", #2
        starttime=starttime, endtime=endtime)
    (batteryVoltage, datesV) = DataAcquisition.get_sensor_data(
        serverUrl=serverUrl,  macId=macId,  browseName="batteryVoltage", #3
        starttime=starttime, endtime=endtime)